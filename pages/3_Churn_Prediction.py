...
from utils import apply_sidebar_filters, build_churn_dataset, load_data
...
with right:
    ...
    fig_roc.add_trace(go.Scatter(
        ...
        line=dict(color="#2E7D68", width=2.5),
    ))
    fig_roc.add_trace(go.Scatter(
        x=rec_curve, y=prec_curve,
        mode="lines",
        name=f"PR (AUC={pr_auc:.3f})",
        line=dict(color="#7A52B3", width=2.5),
    ))
    fig_roc.update_layout(
        title=dict(
            text="ROC & Precision-Recall Curves (held-out 20%)",
            x=0,
            xanchor="left",
            y=0.98,
            yanchor="top",
        ...
        margin=dict(t=56, b=88, l=12, r=12),
        ...
        xaxis=dict(range=[0, 1], showgrid=True, gridcolor="rgba(0,0,0,0.08)", zeroline=False),
        yaxis=dict(range=[0, 1.02], showgrid=True, gridcolor="rgba(0,0,0,0.08)", zeroline=False),
...
st.download_button(
    label="⬇️ Download all churn scores (CSV)",
    data=csv,
